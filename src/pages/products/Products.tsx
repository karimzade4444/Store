...
const Products = () => {
  ...
  return (
    <div className=" grid grid-cols-4 gap-5 p-5 dark:text-foreground">
      {data?.map((el) => (
        <div key={el.id} className="w-80 h-100 bg-card rounded-2xl">
          ...
          <div className="pl-5 pt-2 flex items-center gap-5">
            ...
            <p className="text-black/50 relative dark:text-foreground/50">
              $ {el.price.toFixed(2)}
              <div className="w-full h-0.5 rounded-xl absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/50 z-0 dark:bg-foreground/50"></div>
            </p>
          </div>
          <div className="p-5">
